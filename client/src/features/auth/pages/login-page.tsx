...
import { ArrowLeft, BriefcaseBusiness, Loader2, LogIn, ShieldCheck, UserRound } from 'lucide-react'
    ...
    <main className="min-h-screen bg-base text-primary">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 py-6 sm:px-6 lg:px-8">
        <header className="flex items-center justify-between gap-4">
          <Link to="/" className="inline-flex items-center gap-2 text-sm font-medium text-muted transition hover:text-primary">
        ...
        <section className="grid flex-1 items-center gap-8 py-10 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="hidden lg:block">
            <div className="max-w-md">
              ...
              <p className="mt-4 text-base leading-7 text-muted">
                Access the demo workspace with seeded admin or employee credentials. Accounts are created by admins only.
              </p>
            </div>
          </div>
          ...
          <div className="mx-auto w-full max-w-md rounded-2xl border border-default bg-surface p-5 shadow-card sm:p-6">
            <div className="mb-6">
              ...
              <h2 className="mt-4 text-2xl font-semibold tracking-normal text-primary">Welcome back</h2>
              <p className="mt-2 text-sm leading-6 text-muted">Use an invited account or one of the portfolio demo users.</p>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <Button className="w-full" disabled={loginMutation.isPending} type="submit">
                {loginMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> : <LogIn className="h-4 w-4" aria-hidden="true" />}
                {loginMutation.isPending ? 'Signing in...' : 'Sign in'}
              </Button>
            </form>

            <div className="mt-6 border-t border-subtle pt-5">
              <p className="text-xs font-semibold uppercase tracking-normal text-faint">Demo credentials</p>
              <div className="mt-3 grid gap-3">
                {demoAccounts.map(({ description, email: demoEmail, icon: Icon, label }) => (
                  <button
              ...
              <p className="mt-4 text-xs leading-5 text-muted">Password: <span className="font-mono text-brand-text">{demoPassword}</span></p>
            </div>
          </div>
        </section>
      </div>
