...
export const AttendanceChart = ({ data }: AttendanceChartProps) => {
  ...
  const presentValues = chartData.map((point) => point.present + point.late + point.partial)
  ...
  const presentPath = presentPoints.length > 0 ? `M${toChartPath(presentPoints)}` : ''
  const absentPath = absentPoints.length > 0 ? `M${toChartPath(absentPoints)}` : ''
    ...
    <div className="mt-4 overflow-hidden rounded-xl bg-inset p-3">
      <div className="relative h-56 sm:h-64">
        <svg className="h-full w-full overflow-visible" viewBox="0 0 300 130" role="img" aria-label="Attendance trend line chart">
          ...
          {presentPath ? <path d={`${presentPath} L284 120 L8 120 Z`} fill="url(#dashboard-attendance-fill)" /> : null}
          {absentPath ? <path d={absentPath} fill="none" stroke="var(--chart-secondary)" strokeWidth="2" /> : null}
          {presentPath ? <path d={presentPath} fill="none" stroke="var(--accent-primary)" strokeLinecap="round" strokeWidth="3" /> : null}
        ...
        <div className="absolute inset-x-0 bottom-0 grid text-center text-[10px] font-medium text-muted" style={{ gridTemplateColumns: `repeat(${Math.max(chartData.length, 1)}, minmax(0, 1fr))` }}>
          {chartData.map((point) => (
            <span key={point.date}>{formatDate(`${point.date}T00:00:00`, 'MMM d')}</span>
          ))}
        </div>
      </div>
    </div>
